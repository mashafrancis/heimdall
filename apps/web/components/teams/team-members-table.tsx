...
import { TeamMember } from '@heimdall-logs/types/models';
import {
	ColumnDef,
	ColumnFiltersState,
	SortingState,
...
export const columns: ColumnDef<TeamMember>[] = [
	{
		...
		header: 'Name',
		cell: ({ row }) => (
			<div className='capitalize'>{row.original.name ?? 'unknown'}</div>
		),
	},
	{
		id: 'email',
...
export function TeamMembersTable() {
	...
	const isOwner = () =>
		data?.teamMember.find((u) => u.userId === user?.id)?.role === 'owner';
