...
const transformData = (events: HeimdallEvent[]) => {
  ...
  for (let i = 0; i < hits.length; i++) {
    ...
    const refIndex = referrer.findIndex(
      (a) => a.referrer === transformRef(refDomain),
    )
    ...
    const utmSourceIndex = utmSources.findIndex(
...
const transformRef = (ref: string) => {
  ...
  const url = ref.startsWith('https')
    ? new URL(ref).hostname
    : ref.replace('android-app://', '')
  if (ReferrerName[url]) {
...
export const getVisitorsByDate = (
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    const date = new Date(
      `${event.timestamp.replace(' ', 'T')}Z`,
    ).toLocaleString('default', formatOptions)
