...
export interface FetchInstrumentationConfig extends InstrumentationConfig {
   ...
   * [safaricom URLs](https://heimdall.francismasha.com/docs/safaricom-urls), all
...
export class FetchInstrumentation implements Instrumentation {
  instrumentationName = '@heimdall-logs/nodejs-otel/fetch'
  ...
  public enable(): void {
    ...
    const host =
      process.env.SAFARICOM_URL || process.env.NEXT_PUBLIC_SAFARICOM_URL || null
    ...
    const shouldPropagate = (
      ...
      if (
        host &&
        url.protocol === 'https:' &&
        (url.host === host || url.host === getMxlRequestContext()?.headers.host)
      ) {
        return true
      }
