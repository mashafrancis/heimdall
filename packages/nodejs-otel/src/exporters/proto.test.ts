import type { Attributes, HrTime, Link, SpanStatus } from '@opentelemetry/api'
import { SpanKind, SpanStatusCode, TraceFlags } from '@opentelemetry/api'
import { TraceState, hrTime } from '@opentelemetry/core'
...
describe('OTLP Protobuf', () => {
  ...
  function createSpan({
    ...
    return {
      ...
      ended: true,
      resource,
      instrumentationLibrary: instrumentationScope ?? { name: 'default' },
    ...
    it('should match instrumentationLibrary with schema', () => {
      const span = createSpan({
        instrumentationScope: {
          name: 'lib1',
          version: '1.0.1',
          schemaUrl: 'https://safaricom.co.ke/schema1',
        },
      })
      const request = createExportTraceServiceRequest([span], undefined)
      const expected = encodeViaOtlpLibrary(request)
