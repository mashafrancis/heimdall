...
import { getMxlRequestContext } from '../heimdall-request-context/api'
import { getMxlRequestContextAttributes } from '../heimdall-request-context/attributes'
...
export class CompositeSpanProcessor implements SpanProcessor {
    ...
    return Promise.all(
      this.processors.map((p) =>
        p.forceFlush().catch((e) => {
          diag.error('@heimdall-logs/nodejs-otel: forceFlush failed:', e)
        }),
      ),
    ).then(() => undefined)
  }
  ...
  onStart(span: Span, parentContext: Context): void {
    ...
    if (isRoot && isSampled(traceFlags)) {
      const mrc = getMxlRequestContext()
      const heimdallRequestContextAttrs = getMxlRequestContextAttributes(
        mrc,
        this.attributesFromHeaders,
      )
      if (heimdallRequestContextAttrs) {
        span.setAttributes(heimdallRequestContextAttrs)
      }
        ...
        for (const openSpan of rootObj.open) {
          if (!openSpan.ended && openSpan.spanContext().spanId !== spanId) {
            try {
              ...
              diag.error('@heimdall-logs/nodejs-otel: onEnd failed:', e)
            }
          }
        }
      }
