...
export function FormLayout({
  ...
  // _error is used label && to delay the error string behind removed too soon
  ...
  const dataInputContainerClasses: string[] = []
    ...
    <div className={containerClasses.join(' ')}>
      ...
      {labelled || labelOptional || layout === 'horizontal' ? (
        ...
        >
          {labelled && (
            <label
              className={cn(__styles.label.base, __styles.label.size[size])}
              htmlFor={id}
            >
              {beforeLabel && (
                <span
